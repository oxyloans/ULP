...
export default function TotalAssets() {
  ...
  const status = false; // Placeholder for any status logic if needed
  return (
    <Section title="Total Assets" subtitle={`${totalAssets} assets in lender and company list view`}>
      ...
      <>
        <div className="grid gap-2 md:grid-cols-2">
          {assetValueDistribution.map((item) => (
            <div key={item.label} className="rounded-2xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--border)" }}>
              ...
              <p className="text-xl font-black mt-2" style={{ color: "var(--text-primary)" }}>{(item.value)}</p>
            </div>
          ))}
        </div>
        <TableShell>
        ...
        <div className="text-center py-10" style={{ color: "var(--text-muted)" }}>
          <p className="text-lg font-semibold">No assets data available</p>
          </div>
      )}
    </Section>
  );
