...
function DealsGroup({ item, onClose }) {
  const location = useLocation();
  const isAnyChildActive = DEALS_PATHS.some(p => location.pathname.startsWith(p));
    ...
    <>
      ...
      <nav className="flex-1 px-3 pb-3 flex flex-col gap-0.5 overflow-y-auto">
        {NAV_ITEMS.map(item => {
          ...
          if (item.children) {
            return <DealsGroup key={item.title} item={item} onClose={onClose} />;
          }
          // Coming soon
          if (item.comingSoon) {
            return (
